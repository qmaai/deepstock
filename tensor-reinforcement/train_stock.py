...
# the supervised_y_data shall be in the forms of
# [[[1,port],[0,port],[1,port]...],[],[],..]
# supervised_y_data[i] is one episode.
...
def new_stage_data(action, portfolio, old_state, new_state, portfolio_value, done, episode_data):
    ...
    if action == 1:
        ...
        portfolio += 1
    #selling
    elif action == 2 and portfolio >0:
        #old_price = old_state[2]
         #Todo: Add transaction cost here also 
        portfolio_value += price*portfolio
        portfolio = 0
    elif action ==2 and portfolio <=0:
        portfolio_value += price
        portfolio -=1
