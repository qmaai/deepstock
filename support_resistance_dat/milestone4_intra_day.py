...
# support = into_arr(min_group,tpdata1.as_matrix(),3,True,hard_break_par=0.0005)
...
def main():
    ...
    parser.add_argument('--cluster_threshold_s',type=float,default=0.2,
                        help='the threshold within which data of each price is grouped together, default 0.2')
    parser.add_argument('--cluster_threshold_r',type=float,default=0.2,
                        help='the threshold within which data of each price is grouped together, default 0.2')
    parser.add_argument('--min_points_num_for_line',type=int,default=3,
    ...
    parser.add_argument('--min_points_num_for_cluster',type=int,default=3,
                        help='the min interation to form a line, the more the number, the less the lines, default 3')
    parser.add_argument('--hard_break_par_s',type=float,default=0,
                        help='control break throughs, default 0')
    parser.add_argument('--hard_break_par_r',type=float,default=0,
                        help='control break throughs, default 0')
    ...
    if os.path.exists(args.data_file):
        ...
        # the document cannot handle files that are too long. break into units of 4000
        loops = math.floor(tpdata.shape[0]/4000)
        
        support = []
        resistance =[]
        for i in range(loops):
            tpdata_cur = tpdata[i:i+4000,:]
            # find the local max and min
            maxima,minima = max_min(tpdata_cur,args.scanning_window)
            ...
            # generate the support and resistance line group
            support.extends(into_arr(min_group,tpdata_cur.as_matrix(),args.min_points_num_for_line,True,args.hard_break_par_s))
            resistance.extends(into_arr(max_group,tpdata_cur.as_matrix(),args.min_points_num_for_line,False,args.hard_break_par_r))
        # the last tail parts
        tpdata_cur = tpdata[loops*4000:tpdata.shape[0],:]
        maxima,minima = max_min(tpdata_cur,args.scanning_window)
        max_group = cluster(maxima,args.min_points_num_for_cluster,args.cluster_threshold_s,oneyear=3)
        min_group = cluster(minima,args.min_points_num_for_cluster,args.cluster_threshold_r,oneyear=3)
        support.extends(into_arr(min_group,tpdata_cur.as_matrix(),args.min_points_num_for_line,True,args.hard_break_par_s))
