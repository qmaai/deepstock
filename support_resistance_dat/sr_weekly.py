...
def main():
    ...
    parser.add_argument('--cluster_threshold',type=float,default=4,
                        help='the threshold within which data of each price is grouped together, default 4')
    ...
    if os.path.exists(args.data_file):
        ...
        max_group = cluster(maxima,args.min_points_num_for_cluster,args.cluster_threshold,oneyear=3)
        min_group = cluster(minima,args.min_points_num_for_cluster,args.cluster_threshold,oneyear=3)
...
# date_parser = lambda x: pd.datetime.strptime(x, '%Y-%m-%d %H:%M:%S')
# tpdata = pd.read_table('data/source_txt/@CL_Monthly.txt',delimiter=',',header=0,parse_dates=[['Date','Time']],usecols=['Date','Time','Close'])
# # tpdata=tpdata[200:800]
# # find the local max and min
# maxima,minima = max_min(tpdata,5)
# # print(maxima)
# # print(minima)
# # cluster the max and min to form groups
# max_group = cluster(maxima,3,threshold=4,oneyear=3)
# min_group = cluster(minima,3,threshold=4,oneyear=3)
# print(len(min_group))
# print(len(max_group))
# # generate the support and resistance line group
# support = into_arr(min_group,tpdata.as_matrix(),2,True)
# resistance = into_arr(max_group,tpdata.as_matrix(),2,False)
# # merge the full data with support and resistance
# # the new support and resistance lines shall become extra columns
